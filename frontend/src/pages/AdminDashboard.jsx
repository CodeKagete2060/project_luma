...
export default function AdminDashboard() {
  ...
  const rejectResource = async (id) => {
    ...
    try {
      await api.patch(`/admin/resources/${id}/reject`, { rejectionReason: reason });
      fetchPendingResources();
    ...
    <DashboardLayout role="admin">
      <div>
        ...
        <div className="flex gap-3 mb-6">
          {['analytics', 'pending', 'resources', 'discussions'].map((key) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`px-4 py-2 rounded-lg ${tab === key ? 'bg-primary text-white' : 'bg-white text-gray-700 border'}`}
            >
              {key === 'analytics' ? 'Analytics' : key === 'pending' ? 'Pending Resources' : key === 'resources' ? 'Resource Moderation' : 'Discussion Moderation'}
            </button>
          ))}
        </div>
        ...
        {tab === 'pending' && (
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-semibold mb-4">Pending Resources for Approval</h2>
            {moderationLoading ? (
              ...
              <div className="space-y-3">
                  ...
                  <div key={resource._id} className="border rounded-lg p-4">
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex-1">
                        <div className="font-semibold text-gray-800 text-lg">{resource.title}</div>
                        ...
                        <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                          ...
                          <span>Size: {resource.fileSize ? `${(resource.fileSize / 1024 / 1024).toFixed(2)} MB` : 'N/A'}</span>
                        </div>
                        <div className="text-sm text-gray-500 mt-1">
                          Uploaded by: {resource.uploaderName || resource.uploadedBy?.username || 'Unknown'} on {new Date(resource.uploadDate).toLocaleDateString()}
                        </div>
                ...
                {pendingResources.length === 0 && <p className="text-gray-500 text-sm">No pending resources.</p>}
              </div>
            )}
          </div>
        )}
